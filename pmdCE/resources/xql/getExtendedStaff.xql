...
let $path := concat($freidi-pmd:ce-data,request:get-parameter('path', 'A/A_mov8/A_page128.xml'))
let $staffID := request:get-parameter('staffID', 'A_mov8_measure19_s7')
let $id_prefix := request:get-parameter('id_prefix', 'slurEnd___')
let $endPageName := request:get-parameter('endPageName','A_page.129.xml')
...
let $doc := if($id_prefix = 'slurEnd___')then(doc($endPagePath))else(doc($path))
