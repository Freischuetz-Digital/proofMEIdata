...
-->
  ...
  <!-- mode setupABC -->
    <xsl:template name="getABCHead">
        ...
        <xsl:text>&#xa;M:</xsl:text><xsl:value-of select="local:translateMeter($meter.count,$meter.unit,$meter.sym)"/>
        <xsl:text>&#xa;K:</xsl:text><xsl:value-of select="local:translateKey($key.sig)"/>
        <xsl:text>&#xa;%%barnumbers 1</xsl:text>
        <xsl:text>&#xa;%%measurefirst </xsl:text><xsl:value-of select="(//mei:measure)[1]/@n"/>
        <xsl:text>&#xa;L:1/8</xsl:text>
        <xsl:text>&#xa;%%score </xsl:text><xsl:value-of select="concat(string-join($scoreGroups,' '),codepoints-to-string(13))"/>
